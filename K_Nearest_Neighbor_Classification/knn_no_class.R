...
knnPredict <- function(test_data, train_data, k_value)
{
  ...
  for(i in c(1:nrow(test_data)))
  {
    ...
    for(j in c(1:nrow(train_data)))
    {
      ...
      #eu_char <- c(eu_char, as.character(train_data[j,][[30]]))
    }
    
    eu <- data.frame(eu_dist) #dataframe eu created with eu_char & eu_dist columns
...
K = 1
predictions <- knnPredict(test.norm, train.norm, K) #calling function knnPredict
test.df[,32] <- predictions #adding predictions to the test data as 32nd column
print(paste0("Data is normalized. Accuracy: ", accuracy(test.df), "% for K = ", K, "."))